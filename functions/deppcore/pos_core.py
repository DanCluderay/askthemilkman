...
def add_node_to_loc_grid(para):
    ...
    sqlstring: str = "INSERT INTO fred.Location_Grid(LocName, LocParent, LocType) VALUES ('" + locname + "','" + str(
        locparent) + "','" + str(loctype) + "');"
    print(sqlstring)
    dac_code.db_sql_write(sqlstring)
...
def edit_node_to_loc_grid(para):
    ...
    sqlstring: str = "UPDATE fred.Location_Grid SET LocName = '" + locname + "', LocParent = '" + str(
        locparent) + "' LocType = '" + str(loctype) + "' WHERE LocGridID=" + locid
    print(sqlstring)
    dac_code.db_sql_write(sqlstring)
...
def get_location_Store_Zone_Layout(storeid):
    sqlcode = "SELECT storelayout.id, storelayout.BuildingID, storelayout.LocGrid_ID, storelayout.Control_Type, storelayout.Control_X, storelayout.Control_Y FROM fred.storelayout storelayout WHERE (storelayout.BuildingID = " + str(
        storeid) + ")"
    result = dac_code.dbreadquery_sql(sqlcode)
    return result
...
def add_store_layout_row(para):
    ...
    sqlstring: str = "INSERT INTO fred.storelayout(BuildingID, LocGrid_ID, Control_Type, Control_Y, Control_X, Control_Z) VALUES (" + str(
        BuildingID) + "," + str(LocGrid_ID) + "," + str(Control_Type) + "," + str(Control_Y) + "," + str(
        Control_X) + "," + str(Control_Z) + ");"
    print(sqlstring)
    dac_code.db_sql_write(sqlstring)
...
def edit__store_layout_row(para):
    ...
    sqlstring: str = "UPDATE fred.storelayout SET BuildingID =" + str(BuildingID) + ", LocGrid_ID =" + str(
        LocGrid_ID) + ", Control_Type =" + str(Control_Type) + ", Control_Y =" + str(Control_X) + ", Control_X =" + str(
        Control_Y) + ", Control_Z =" + str(Control_Z) + " WHERE id=" + str(id)
